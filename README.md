...
BATCH SCHEDULER controls the computational resources.  This is chosen by the administrator of your computer.
                Examples include SLURM and PBS.
...
Users also need jobs script.  These scripts are what actually gets submitted to the scheduler.  So, it needs to have all the usual batch scheduler mark up.  It needs to convert the batch scheduler variables to METAQ variables.  Then, it executes METAQ/x/launch.sh, which is where all the magic happens.
...
METAQ/x/launch.sh logs the individual tasks separately in METAQ/jobs/${job_id}/log/${task}.log and tallies the available resources in jobs/${job_id}/resources, where you can see which tasks got started when, and what resources they consumed.  My personal preference is to direct the output log of METAQ/x/launch.sh itself to jobs/${job_id}/log/ as well, but that is specified in the job script.
...
Task scripts inherit the bash environment variables, but do not have access to METAQ variables, and are not passed any parameters.  They should be relatively self-contained.
...
Job scripts are bash scripts that get submitted to your batch scheduler (SLURM, PBS, etc.).  The structure of a job script is as follows.  Job scripts MUST be bash (sorry).
...
#MSUB -o /p/lscratche/berkowit/metaq/log/%J.log
#MSUB -N metaq_job
...
However, it may be that your binary is smart and knows how make use of all the resources on the whole physical node at once.  In that case why are you overcommitting?---and you can again forget about GPUs and just worry about NODEs.
...
Jobs in the priority folder are only preferred at the beginning of a job.